...
class NeuralIBM1Model:
  ...
  def _create_weights(self):
    ...
    self.W_t = tf.get_variable(name="W_t", initializer=glorot_uniform(), 
                                shape=[self.mlp_dim, self.y_vocabulary_size])# |V_F| x d_h
    self.b_t = tf.get_variable(name="b_t", initializer=tf.zeros_initializer(), 
                                shape=[self.y_vocabulary_size])# |V_F| = y_vocabulary_size
    self.W_h_e = tf.get_variable(name="W_h_e", initializer=glorot_uniform(), 
                                shape=[self.emb_dim, self.mlp_dim])# d_h x d_e
    self.b_h_e = tf.get_variable(name="b_h_e", initializer=tf.zeros_initializer(), 
                                shape=[self.mlp_dim])# d_h = mlp dim
    # self.W_r_e = tf.get_variable(name="W_r_e", initializer=glorot_uniform(), 
    #                             shape=[self.emb_dim, self.x_vocabulary_size])# d_e x |V_E|
  ...
  def _build_model(self):
    ...
    print("X_EMBEDDINGS:", x_embeddings.get_shape())
    x_embedded = tf.gather(x_embeddings, self.x) # To do
    print("x_embedded:", x_embedded.get_shape(), self.batch_size)
    ...
    print("batch:", batch_size, "longest_x:", longest_x, 
          "longest_y:", longest_y)
    ...
    # lengths = tf.cast(tf.expand_dims(x_len, -1), tf.float32)  # Shape: [B, ?] # To do
    lengths = tf.cast(x_len, tf.float32)
    print('lengths:', lengths)
    pa_x = tf.math.divide(x_mask, lengths)  # Shape: [B, ?] # To do
    print("pa_x", pa_x.get_shape())

    # We now have a matrix with 1/? values.
    # But later we will need it N times
    pa_x = tf.expand_dims(pa_x, axis=1)  # Shape: [B, 1, M] # To do
    print("pa_x", pa_x.get_shape())

    # Now we perform the tiling:
    pa_x = tf.tile(pa_x, [1, longest_y, 1])  # [B, N, M] # To do
    print("pa_x", pa_x.get_shape())
    ...
    print("mlp:", self.mlp_dim, "emb:", self.emb_dim, "x", self.x_vocabulary_size, 
          "y", self.y_vocabulary_size)
    #r_e = tf.matmul(self.W_r_e, mlp_input)
    #print("r_e:", r_e.get_shape(), "W_r_e:", self.W_r_e.get_shape(), "mlp_input:", mlp_input.get_shape())
    h = tf.tanh(
          tf.matmul(mlp_input, self.W_h_e) + self.b_h_e # To do
        )
    h = tf.matmul(h, self.W_t) + self.b_t
    print("h", h.get_shape())

    # Now we perform a softmax which operates on a per-row basis.
    py_xa = tf.nn.softmax(h) # To Do
